/**
 * Parser for cards-services block variant
 * Extracts service cards with pictogram icons from AT&T Business pages
 */
export default function parse(element, { document }) {
  // Find all service card items
  const cardItems = element.querySelectorAll('.pictogram-card, [class*="pictogram-item"], .service-card, [class*="card"]');

  const cells = [];

  cardItems.forEach(card => {
    // Extract icon/pictogram image
